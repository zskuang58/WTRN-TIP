    ...
    def __init__(self, num_res_blocks, n_feats, res_scale):
        ...
        for i in range(self.num_res_blocks):
            self.RBs.append(ResBlock(
                in_channels=n_feats, out_channels=n_feats,
                res_scale=res_scale
            ))
...
class WaveCross(nn.Module):
    def __init__(self, n_feats):
        ...
        for _ in range(self.num_res_blocks):
            self.LH_RB.append(
                ResBlock(in_channels=n_feats, out_channels=n_feats)
            )
            self.HL_RB.append(
                ResBlock(in_channels=n_feats, out_channels=n_feats)
            )
...
class MainNet(nn.Module):
    ...
    def forward(self, x, S=None, T_lv3=None,
                T_lv2=None, T_lv1=None, skips_lr=None, skips_T=None):
        ...
        x23 = self.conv23_head(skips_T['T_lv2'][0]) * F.interpolate(
            S, scale_factor=2, mode='bicubic'
        )
        x24 = self.conv24_head(skips_T['T_lv2'][1]) * F.interpolate(
            S, scale_factor=2, mode='bicubic'
        )
        x25 = self.conv25_head(skips_T['T_lv2'][2]) * F.interpolate(
            S, scale_factor=2, mode='bicubic'
        )
