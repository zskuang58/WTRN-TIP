...
class WaveCross(nn.Module):
    def __init__(self, n_feats):
        super(WaveCross, self).__init__()
        self.LH_RB = nn.ModuleList()
        self.HL_RB = nn.ModuleList()
        self.HH_RB = nn.ModuleList()
        
        self.num_res_blocks = 2

        for _ in range(self.num_res_blocks):
            self.LH_RB.append(ResBlock(in_channels=n_feats, out_channels=n_feats))
            self.HL_RB.append(ResBlock(in_channels=n_feats, out_channels=n_feats))
            self.HH_RB.append(ResBlock(in_channels=n_feats, out_channels=n_feats))

        self.convLH_merge = nn.ModuleList()     
        self.convHL_merge = nn.ModuleList()     
        self.convHH_merge = nn.ModuleList() 

        for _ in range(self.num_res_blocks):
            self.convLH_merge.append(conv1x1(n_feats*3, n_feats))
            self.convHL_merge.append(conv1x1(n_feats*3, n_feats))
            self.convHH_merge.append(conv1x1(n_feats*3, n_feats))

        self.convLH_tail = conv3x3(n_feats, n_feats)
        self.convHL_tail = conv3x3(n_feats, n_feats)
        self.convHH_tail = conv3x3(n_feats, n_feats) 
    ...
    def forward(self, lh, hl, hh):
        ...
        for i in range(self.num_res_blocks):
            lh_res = self.LH_RB[i](lh_res)
            hl_res = self.HL_RB[i](hl_res)
            hh_res = self.HH_RB[i](hh_res)
        
        lh_res = self.convLH_tail(lh_res)
        hl_res = self.convHL_tail(hl_res)
        hh_res = self.convHH_tail(hh_res)
...
class MainNet(nn.Module):
    def __init__(self, num_res_blocks, n_feats, res_scale):
        ...
        self.conv12_head = conv3x3(128, n_feats)
        self.conv13_head = conv3x3(128, n_feats)
        self.conv14_head = conv3x3(128, n_feats)
        ...
        self.wave_cross1 = WaveCross(n_feats=n_feats)
        
        
        self.conv11_tail = conv3x3(n_feats, n_feats)
        self.unpool1 = wavelet.WaveUnpool(n_feats)
        self.conv15_head = conv3x3(64*4, n_feats)
        self.convps12 = conv3x3(n_feats, n_feats*4)
        self.ps12 = nn.PixelShuffle(2)
        ### stage22
        
        self.conv22_head = conv3x3(128+n_feats, n_feats)
        self.conv23_head = conv3x3(n_feats, n_feats)
        self.conv24_head = conv3x3(n_feats, n_feats)
        self.conv25_head = conv3x3(n_feats, n_feats)
        ...
        self.wave_cross2 = WaveCross(n_feats=n_feats)
        
        
        self.conv21_tail = conv3x3(n_feats, n_feats)
        self.conv22_tail = conv3x3(n_feats, n_feats)
         
        self.unpool2 = wavelet.WaveUnpool(n_feats)
        self.conv26_head = conv3x3(64*4, n_feats)
        self.convps23 = conv3x3(n_feats, n_feats*4)
    ...
    def forward(self, x, S=None, T_lv3=None, T_lv2=None, T_lv1=None, skips_lr=None, skips_T=None):

        x_inter = F.interpolate(x, scale_factor=4, mode='bicubic')
        x_inter = torch.clamp(x_inter, -1, 1)
        ...
        x12 = self.conv12_head(skips_T['T_lv3'][0]) * S
        x13 = self.conv13_head(skips_T['T_lv3'][1]) * S
        x14 = self.conv14_head(skips_T['T_lv3'][2]) * S

        x12, x13, x14 = self.wave_cross1(x12, x13, x14)
        ...
        x21 = x11
        x21_res = x21
        x22 = self.convps12(x11)
        x22 = F.relu(self.ps12(x22))
        #进行waveUnpool，upsample*2尺度

        x22 = self.unpool1(x22, x12, x13, x14)
        x22 = self.conv15_head(x22)
        ### soft-attention
        x22_res = x22
        ...
        x23 = self.conv23_head(skips_T['T_lv2'][0]) * F.interpolate(S, scale_factor=2, mode='bicubic')
        x24 = self.conv24_head(skips_T['T_lv2'][1]) * F.interpolate(S, scale_factor=2, mode='bicubic')
        x25 = self.conv25_head(skips_T['T_lv2'][2]) * F.interpolate(S, scale_factor=2, mode='bicubic')

        x23, x24, x25 = self.wave_cross2(x23, x24, x25) 
        ...
        x32 = x22
        x32_res = x32
        x33 = self.convps23(x22)
        x33 = F.relu(self.ps23(x33))
        x33 = self.unpool2(x33, x23, x24, x25)
        x33 = self.conv26_head(x33)
