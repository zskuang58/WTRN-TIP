...
class SearchTransfer(nn.Module):
    ...
    def forward(self, lrsr_lv3, refsr_lv3, ref_lv3,ref_lv2,ref_lv1, skips):
        ...
        for component in [0,1,2]:
            refskips_lv3_unfold = F.unfold(skips['pool2'][component], kernel_size=(3, 3), padding=1)
            Tskips_lv3_unfold = self.bis(refskips_lv3_unfold, 2, R_lv3_star_arg)  # [N, C*k*k, H*W]
            temp_T_lv3 = F.fold(Tskips_lv3_unfold, output_size=lrsr_lv3.size()[-2:], kernel_size=(3, 3), padding=1) / (3. * 3.)
            skips_T['T_lv3'].append(temp_T_lv3)

            refskips_lv2_unfold = F.unfold(skips['pool1'][component], kernel_size=(6, 6), padding=2, stride=2)
            Tskips_lv2_unfold = self.bis(refskips_lv2_unfold, 2, R_lv3_star_arg)
            temp_T_lv2 = F.fold(Tskips_lv2_unfold, output_size=(lrsr_lv3.size(2) * 2, lrsr_lv3.size(3) * 2), kernel_size=(6, 6),
