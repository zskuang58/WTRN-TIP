...
class Trainer():
    ...
    def train(self, current_epoch=0, is_init=False):
        ...
        for i_batch, sample_batched in enumerate(self.dataloader['train']):
            ...
            sr, S, T_lv3, T_lv2, T_lv1,skips_T = self.model(lr=lr, lrsr=lr_sr, ref=ref, refsr=ref_sr)
            ...
            if (is_init):
                ...
                rec_loss = self.args.rec_w * self.loss_all['rec_ll_loss'](sr, hr)
                loss = rec_loss
            if (is_print):
                self.logger.info( ('init ' if is_init else '') + 'epoch: ' + str(current_epoch) + 
                    '\t batch: ' + str(i_batch+1) )
                self.logger.info( 'rec_loss: %.10f' %(rec_loss.item()) )
                self.tb_logger.add_scalar('losses/rec_loss', rec_loss.item(), self.iter_idx)
            if (not is_init):
                if ('per_loss' in self.loss_all):
                    ...
                    if (is_print):
                        ...
                        self.tb_logger.add_scalar('losses/per_loss', per_loss.item(), self.iter_idx)
                if ('tpl_loss' in self.loss_all):
                    sr_lv1, sr_lv2, sr_lv3,sr_skips = self.model(sr=sr)
                    res1,res2 = self.loss_all['tpl_loss'](sr_lv3,sr_lv2,sr_lv1, sr_skips, 
                        S, T_lv3,T_lv2,T_lv1, skips_T)
                    tpl_loss = 1e-3 * res1 + self.args.tpl_w * res2
                    loss += tpl_loss
                    if (is_print):
                        self.logger.info( 'tpl_loss: %.10f' %(tpl_loss.item()) )
                        self.tb_logger.add_scalar('losses/tpl_loss', tpl_loss.item(), self.iter_idx)
                if ('adv_loss' in self.loss_all):
                    adv_loss = self.args.adv_w * self.loss_all['adv_loss'](sr, hr,ref)
                    loss += adv_loss
                    if (is_print):
                        self.logger.info( 'adv_loss: %.10f' %(adv_loss.item()) )
                        self.tb_logger.add_scalar('losses/adv_loss', adv_loss.item(), self.iter_idx)
            loss.backward()
            self.optimizer.step()
            self.iter_idx += 1
    ...
    def evaluate(self, current_epoch=0):
        ...
        if (self.args.dataset == 'CUFED'):
            ...
            with torch.no_grad():
                psnr, ssim, cnt = 0., 0., 0
                lpips = 0.
                test_data_bar = tqdm(self.dataloader['test']['6'])
                for i_batch, sample_batched in enumerate(test_data_bar):
                    ...
                    ref = sample_batched['Ref']
                    ref_sr = sample_batched['Ref_sr']
                    sr, _, _, _, _, _ = self.model(lr=lr, lrsr=lr_sr, ref=ref, refsr=ref_sr)
                ...
                self.tb_logger.add_scalar('metrics/psnr', psnr_ave, current_epoch)
                self.tb_logger.add_scalar('metrics/ssim', ssim_ave, current_epoch)
                self.tb_logger.add_scalar('metrics/lpips', lpips_ave, current_epoch)
                if (psnr_ave > self.max_psnr):
                    ...
                    if self.args.save_model:
                        ...
                        model_state_dict = {key.replace('module.',''): tmp[key] for key in tmp if 
                            (('SearchNet' not in key) and ('_copy' not in key))}
                        model_name = self.args.save_dir.strip('/')+'/model/model_best.pth'
                        torch.save(model_state_dict, model_name)
    ...
    def test(self):
        ...
        self.model.eval()
        with torch.no_grad():
            sr, _, _, _, _,_ = self.model(lr=LR_t, lrsr=LR_sr_t, ref=Ref_t, refsr=Ref_sr_t)
            sr_save = (sr+1.) * 127.5
            sr_save = np.transpose(sr_save.squeeze().round().cpu().numpy(), (1, 2, 0)).astype(np.uint8)
